...
def validate_bundle(root: Path) -> list[str]:
    ...
    if not exe.is_file(): errors.append(f"missing executable: {exe}")
    for name in ("ffmpeg.exe", "ffprobe.exe"):
        if not (root/name).is_file(): errors.append(f"missing bundled binary: {name}")
    assets = root / "assets" / "logo.png"
    if not assets.is_file(): errors.append("missing assets/logo.png")
    return errors
