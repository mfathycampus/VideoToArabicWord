...
class TranscriptionConfig(BaseModel):
    ...
    # سدّ الفجوات: كل فجوة بلا تفريغ أطول من ``gap_refill_min_seconds``
    # وفيها طاقة صوتية تُعاد بلا VAD، على تلك الفجوة وحدها.
    #
    # **مقيس** على «متابعة تحضير المعلمين» (5:12، صوتٌ مُطبَّع):
    #
    #     الإعداد              تغطية   ك/د    أطول فجوة   زمن
    #     الحالي (VAD 0.5)     25.5٪   23.4   88.9 ث     147 ث
    #     VAD 0.35             31.5٪   30.9   86.5 ث     195 ث
    #     بلا hallucination    25.5٪   23.4   88.9 ث     144 ث
    #     beam 5               26.0٪   25.1   88.9 ث     154 ث
    #     VAD معطَّل           63.7٪   61.8   11.8 ث     366 ث
    #     VAD + سدّ الفجوات    47.4٪   45.3   22.8 ث     430 ث
    #
    # السدّ يسترجع 3:13–4:42 كلامًا حقيقيًّا بلا حلقات هلوسة (الـ63.7٪
    # بلا VAD تشمل حلقتين مطويّتين). كلفته تتبع حجم الفجوات: ملفّ يلتقط
    # فيه VAD الكلام جيدًا لا يدفع شيئًا تقريبًا، وتسجيلٌ هادئ كهذا
    # يدفع ~×3 من زمن التفريغ — ثمن ثلاثة أرباع المحاضرة.
    #
    ...
    gap_refill_min_rms_dbfs: float = -45.0
...
class StudySettings(BaseModel):
    ...
    #: إن تعذّر مزوّد الحزمة واستُعمل مزوّدٌ متاح لإعادة الصياغة في المهمة
    #: نفسها، تُبنى الحزمة به. رُصد: «ollama غير متاح» ⇒ 0 أهداف و0 أسئلة،
    #: بينما Claude يعمل في المهمة ذاتها. السياسة تُطبَّق على مزوّد الصياغة
    #: عند التحميل (``config/policy.py``)، فالبديل لا يتجاوزها.
    use_rewrite_provider_as_fallback: bool = True
