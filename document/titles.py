...
# ── بطاقات المشاركين في الاجتماع: رابع ما يقرؤه الـOCR وليس محتوى ──────
#
# رُصد على مخرج حقيقي من تسجيل اجتماع (شاشة مشتركة + بطاقات فيديو
# المشاركين على الحافة) عنوانان صورةٍ خرجا فعلًا إلى المستند:
#
#   «Hanaa... Z»                 ← بطاقة فيديو: اسمٌ مقصوص ثم حرف الكنية
#   «Shoub... Z Ali Ma... Z»     ← بطاقتان متجاورتان في سطر OCR واحد
#
# كلاهما مرّ من كل الفحوص السابقة: ليس ترتيبيًا، ولا مسار إنترنت، ولا
# شريط متصفّح، وطوله كافٍ بعد قصّ الشظيّة الأخيرة («٢ ٦ Hanaa... Z» ←
# قصّ « Z» كان سينزل تحت الحدّ الأدنى فتراجع القصّ وأبقى العنوان كما
# هو). لا حلّ إلا فحصٌ مباشر: هل السطر **كله** أسماءً مقصوصة بثلاث نقاط
# وأرقام ترقيم شبكة وحروف كنية، ولا كلمة حقيقية واحدة فيه؟
_VIDEO_TILE_TOKEN = (
    r"(?:\d+"
    r"|[A-Za-z؀-ۿ]{1,14}\.\.\.[‎‏]?"
    r"|[A-Za-z؀-ۿ]{1,4}[‎‏]?"
...
_VIDEO_TILE_NAME = re.compile(
    rf"^(?=.*\.\.\.)\s*{_VIDEO_TILE_TOKEN}(?:\s+{_VIDEO_TILE_TOKEN})*\s*$")
...
def title_defects(candidate: str, min_chars: int = JUDGE_MIN_CHARS) -> List[str]:
    ...
    if _VIDEO_TILE_NAME.match(candidate):
        defects.append("بطاقة اسم مشارك")
