...
def strip_source_fingerprint(name: str) -> str:
    """يُزيل بصمة مصدر الملف (``__<hex>``) من اسم مجلد المهمة للعرض.

    ``core/pipeline.py::job_dir_for`` يُلحق بصمة سداسية عشرية باسم مجلد
    المهمة لمنع تصادم ملفين بنفس الاسم من مسارين مختلفين. البصمة ضرورية
    على القرص لتمييز المهام، لكنها ضجيج بصري للمستخدم — هذه الدالة هي
    نقطة الفصل بين "اسم المجلد" و"الاسم المعروض".

    **الطول يُقرأ من ``utils.fingerprints`` لا يُكتب هنا.** كان مثبّتًا
    على 8 بينما صارت البصمة 20 حرفًا، فتوقّف التنظيف صامتًا وظهرت
    أسماء مثل ``Lecture__46b8dd1239bce8b8c7db`` في قائمة المهام وفي
    عناوين المستندات المدموجة. الحدّ الأدنى 8 يُبقي مهام قديمة
    أُنشئت بالطول السابق قابلة للتنظيف.
    """
    import re

    from utils.fingerprints import FINGERPRINT_LENGTH

    pattern = rf"__[0-9a-f]{{8,{max(8, FINGERPRINT_LENGTH)}}}\b"
